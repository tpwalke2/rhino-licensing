...
namespace Rhino.Licensing.Wcf
{
    ...
    {
        private readonly List<ILicenseValidator> availableLicenses = new List<ILicenseValidator>();
        private readonly Dictionary<string, KeyValuePair<DateTime, ILicenseValidator>> leasedLicenses = new Dictionary<string, KeyValuePair<DateTime, ILicenseValidator>>();
        ...
        public LicensingService()
        {
            ...
            if (LicenseValidatorFactory == null)
                throw new InvalidOperationException("LicenseValidatorFactory must be set before starting the service");
        ...
        /// <summary>
        /// Gets or Sets the license validator factory
        /// </summary>
        public static ILicenseValidatorFactory LicenseValidatorFactory
        {
            get; set;
        }
        ...
        private void ReadAvailableLicenses(string licensesDirectory)
        {
            foreach (var license in Directory.GetFiles(licensesDirectory, "*.xml"))
            {
                ...
                var validator = LicenseValidatorFactory.Create(SoftwarePublicKey, license, true);
                try
                {
            ...
            try
            {
                using (var binaryReader = new BinaryReader(stream))
                {
                    while (true)
                    {
                        ...
                        leasedLicenses[identifier] = new KeyValuePair<DateTime, ILicenseValidator>(time, licenseValidator);
		...
		public string LeaseLicense(
			...
			Guid id)
		{
			KeyValuePair<DateTime, ILicenseValidator> value;
		...
		private string GenerateLicenseAndRenewLease(string identifier, Guid id, ILicenseValidator licenseValidator, IDictionary<string, string> attributes)
		{
			leasedLicenses[identifier] = new KeyValuePair<DateTime, ILicenseValidator>(DateTime.UtcNow.AddMinutes(30), licenseValidator);
		...
		private static string GenerateLicense(Guid id, ILicenseValidator validator, IDictionary<string, string> attributes)
		{
